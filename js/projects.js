let induvidualProjectData = {
    ...
    android: {
        "EXAMM-SNAPP": {
            ...
            techStack: {
                0: "/assets/dart.png",
                1: "/assets/flutter.png",
                2: "/assets/firebase.png",
            },
        }, 
...
function addEventListeners() {
    document.querySelector(".web > ul > li:nth-child(1)").addEventListener("click", renderContent("web", "worksy"));
    document.querySelector(".web > ul > li:nth-child(2)").addEventListener("click", renderContent("web", "Bus-Track"));
    document.querySelector(".web > ul > li:nth-child(3)").addEventListener("click", renderContent("web", "Tile-Game"));
    document.querySelector(".web > ul > li:nth-child(4)").addEventListener("click", renderContent("web", "API-EXPERIMENTS"));
...
function renderContent(projSec, projName){
    return function () {
        ...
        renderGallery(projSec, projName, "techStack");
        renderGallery(projSec, projName, "addnlPics");
    }
}

function makeGallery(count, imgGrp) {
    let addLi = document.createElement("li");
    let addImg = document.createElement("img");
    document.querySelector(`.${imgGrp}`).appendChild(addLi);
    document.querySelector(`.${imgGrp} > li:nth-child(${count})`).appendChild(addImg);
}

function renderGallery(projSec, projName, imgGrp) {
    let count = 1;
    for( ele in induvidualProjectData[projSec][projName][imgGrp]) {
        makeGallery(count, imgGrp);
        document.querySelector(`.${imgGrp} > li:nth-child(${count}) > img`).src = induvidualProjectData[projSec][projName][imgGrp][count - 1];
        count = count + 1;
    }
}